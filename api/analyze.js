export default async function handler(req,res){
  ...
  if(!channelData||!videoStats)return res.status(400).json({error:'channelData dan videoStats diperlukan'});
  const AI_BASE_URL=process.env.AI_BASE_URL;
  const AI_MODEL=process.env.AI_MODEL;
  const AI_API_KEY=process.env.AI_API_KEY;

  const prompt=`Berikut adalah data channel YouTube:
...
Jawab secara ringkas tapi profesional, gunakan bahasa Indonesia.`;
  ...
  try{
    const response=await fetch(`${AI_BASE_URL}/chat/completions`,{
      ...
      body:JSON.stringify({model:AI_MODEL,messages:[{role:"user",content:prompt}],max_tokens:1000})
    });
    const data=await response.json();
    const aiText=data.choices?.[0]?.message?.content||'Tidak ada hasil dari AI';
    res.status(200).json({aiText});
  }catch(err){console.error('Gemini AI error:',err);res.status(500).json({error:'Terjadi kesalahan saat memanggil AI'});}
}
